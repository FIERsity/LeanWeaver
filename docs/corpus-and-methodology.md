...
> 纯规则、零 LLM 的 Lean 4 报错解释器——用**官方验证过的真实报错**构建，
> 不依赖任何大模型，每个解释都可溯源、可复现、离线可用。

## 一句话

Lean 4 的报错对新手极不友好，而**官方扩展只显示原始报错，不做解释**。
LeanWeaver 把报错翻译成人话（含义 + 原因 + 修复），
且**语料全部来自 Lean 官方测试**，不是 AI 编的。
...
### 识别率提升轨迹（数据驱动）
```
初始（手工 20 类）    → 46.2%   ← 第一次对官方语料测
补 infer/synthesize   → 57.3%   ← 补 3 类
补 declared/ambiguous → 66.3%   ← 补 5 类
补 no_progress/unterm → 68.2%   ← 补 4 类
复核修复误配         → 66.0%   ← 诚实修正（去掉虚高）
```
**每轮补规则都有官方语料支撑**（不是拍脑袋），且复核发现并修复了
"`expected` 兜底过宽误配"的问题。
...
2. **解释文本最初由 LLM 辅助编写**，已固化进规则库，经人工复核；
